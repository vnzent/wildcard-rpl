    ...
    public function handle(): void
    {
        ...
        if (config('filament-cms.youtube_key')) {
            ...
            if (isset($youtube['items']) && count($youtube['items'])) {
                ...
                Notification::make()
                    ->title(trans('cms.content.posts.import.youtube.notifications.title'))
                    ->body(trans('cms.content.posts.import.youtube.notifications.description', ['name' => $post->title]))
                    ->success()
                    ->actions([
                        Action::make('view')
                            ->label(trans('cms.content.posts.import.youtube.notifications.view'))
                ...
                Notification::make()
                    ->title(trans('cms.content.posts.import.youtube.notifications.failed_title'))
                    ->body(trans('cms.content.posts.import.youtube.notifications.failed_description', ['name' => $this->url]))
                    ->danger()
                    ->sendToDatabase($user);
            }
