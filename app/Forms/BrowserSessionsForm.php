...
class BrowserSessionsForm
{
    ...
    {
        return [
            Section::make(trans('account.profile.browser.browser_section_title'))
                ->description(trans('account.profile.browser.browser_section_description'))
                ->schema([
                    Forms\Components\ViewField::make('browserSessions')
                        ->label(__(trans('account.profile.browser.browser_section_title')))
                        ->hiddenLabel()
                    ...
                    Actions::make([
                        Actions\Action::make('deleteBrowserSessions')
                            ->label(trans('account.profile.browser.browser_sessions_log_out'))
                            ->requiresConfirmation()
                            ->modalHeading(trans('account.profile.browser.browser_sessions_log_out'))
                            ->modalDescription(trans('account.profile.browser.browser_sessions_confirm_pass'))
                            ->modalSubmitActionLabel(trans('account.profile.browser.browser_sessions_log_out'))
                            ->form([
                                Forms\Components\TextInput::make('password')
                                    ->password()
                                    ->revealable()
                                    ->label(trans('account.profile.browser.password'))
                                    ->required(),
                            ])
    ...
    public static function logoutOtherBrowserSessions($password): void
    {
        ...
        if (! Hash::check($password, Auth::user()->password)) {
            Notification::make()
                ->danger()
                ->title(trans('account.profile.browser.incorrect_password'))
                ->send();
