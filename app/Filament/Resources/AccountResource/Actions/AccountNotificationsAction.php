...
class AccountNotificationsAction
{
    public static function make(): Action
    {
        return Action::make('notify')
            ->label(trans('account.accounts.actions.notifications'))
            ->icon('heroicon-s-bell')
            ->iconButton()
            ->tooltip(trans('account.accounts.actions.notifications'))
            ->color('info')
            ->form(function ($record) {
                return [
                    Forms\Components\Toggle::make('use_notification_template')
                        ->label(trans('account.accounts.notifications.use_notification_template'))
                        ->default(true)
                        ->live()
                        ->required(),
                    Forms\Components\Select::make('template_id')
                        ->label(trans('account.accounts.notifications.template_id'))
                        ...
                        ->required(),
                    Forms\Components\SpatieMediaLibraryFileUpload::make('image')
                        ->label(trans('account.accounts.notifications.image'))
                        ->hidden(fn (Forms\Get $get) => $get('use_notification_template'))
                        ->collection('images')
                        ->image(),
                    Forms\Components\TextInput::make('title')
                        ->label(trans('account.accounts.notifications.title'))
                        ->hidden(fn (Forms\Get $get) => $get('use_notification_template'))
                        ->label(trans('filament-alerts::messages.templates.form.title'))
                        ->required()
                        ->maxLength(255),
                    Forms\Components\Textarea::make('body')
                        ->label(trans('account.accounts.notifications.body'))
                        ->hidden(fn (Forms\Get $get) => $get('use_notification_template'))
                        ->label(trans('filament-alerts::messages.templates.form.body'))
                        ->columnSpanFull(),
                    Forms\Components\TextInput::make('url')
                        ->label(trans('account.accounts.notifications.url'))
                        ->hidden(fn (Forms\Get $get) => $get('use_notification_template'))
                        ->label(trans('filament-alerts::messages.templates.form.url'))
                        ->url()
                        ->maxLength(255),
                    IconPicker::make('icon')
                        ->label(trans('account.accounts.notifications.icon'))
                        ->hidden(fn (Forms\Get $get) => $get('use_notification_template'))
                        ->label(trans('filament-alerts::messages.templates.form.icon'))
                        ->default('heroicon-o-check-circle'),
                    Forms\Components\Select::make('type')
                        ->label(trans('account.accounts.notifications.type'))
                        ->hidden(fn (Forms\Get $get) => $get('use_notification_template'))
                        ->label(trans('filament-alerts::messages.templates.form.type'))
                        ->options(collect(config('filament-alerts.types'))->pluck('name', 'id')->toArray())
                        ->default('success'),
                    Forms\Components\Select::make('providers')
                        ->label(trans('account.accounts.notifications.providers'))
                        ->hidden(fn (Forms\Get $get) => $get('use_notification_template'))
                        ->label(trans('filament-alerts::messages.templates.form.providers'))
                        ->multiple()
                        ->options(collect(config('filament-alerts.providers'))->pluck('name', 'id')->toArray()),
