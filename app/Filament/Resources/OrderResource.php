...
class OrderResource extends Resource
{
    ...
    {
        return trans('ecommerce.orders.title'); // TODO: Change the autogenerated stub
    }

    public static function getPluralLabel(): ?string
    {
        return trans('ecommerce.orders.title');
    }

    public static function getLabel(): ?string
    {
        return trans('ecommerce.orders.single'); // TODO: Change the autogenerated stub
    }
        ...
        return $form
            ->schema([
                Forms\Components\TextInput::make('uuid')
                    ...
                    ->label(trans('ecommerce.orders.columns.uuid'))
                ...
                Forms\Components\Grid::make([
                    ...
                    Forms\Components\Section::make(trans('ecommerce.orders.sections.company'))
                        ->schema([
                            Forms\Components\Select::make('company_id')
                                ...
                                ->live()
                                ->required()
                                ->label(trans('ecommerce.orders.columns.company_id')),
                            Forms\Components\Select::make('branch_id')
                                ...
                                ->label(trans('ecommerce.orders.columns.branch_id')),
                            Forms\Components\Select::make('status')
                                ->label(trans('ecommerce.orders.columns.status'))
                                ->searchable()
                                ->preload()
                            ...
                            Forms\Components\Select::make('payment_method')
                                ...
                                ->default('cash')
                                ->label(trans('ecommerce.orders.columns.payment_method')),
                        ])->columns(2)->collapsible()->collapsed(fn ($record) => $record),
                    Forms\Components\Section::make(trans('ecommerce.orders.sections.account'))
                        ->schema([
                            Forms\Components\Select::make('account_id')
                                ...
                                ->label(trans('ecommerce.orders.columns.account_id'))
                                ->required(),
                            Forms\Components\TextInput::make('name')
                                ->label(trans('ecommerce.orders.columns.name'))
                                ->maxLength(255),
                            Forms\Components\TextInput::make('phone')
                                ->label(trans('ecommerce.orders.columns.phone'))
                                ->maxLength(255),
                            Forms\Components\Select::make('source')
                                ->searchable()
                                ->options(Type::query()->where('for', 'orders')->where('type', 'source')->pluck('name', 'key')->toArray())
                                ->label(trans('ecommerce.orders.columns.source'))
                                ->required()
                                ->default('system'),
                        ])->columnSpan(6)->collapsible()->collapsed(fn ($record) => $record),
                    Forms\Components\Section::make(trans('ecommerce.orders.sections.location'))
                        ->schema([
                            ...
                            Forms\Components\TextInput::make('flat')
                                ->label(trans('ecommerce.orders.columns.flat'))
                                ->columnSpanFull()
                                ->maxLength(255),
                            Forms\Components\Textarea::make('address')
                                ->label(trans('ecommerce.orders.columns.address'))
                                ->columnSpanFull(),
                        ])->columns(2)->columnSpan(6)->collapsible()->collapsed(fn ($record) => $record),
                ]),
                Forms\Components\Section::make(trans('ecommerce.orders.sections.items'))
                    ->schema([
                        Forms\Components\Repeater::make('items')
                            ->hiddenLabel()
                            ->label(trans('ecommerce.orders.columns.items'))
                            ->schema([
                                Forms\Components\Select::make('product_id')
                                    ...
                                    ->label(trans('ecommerce.orders.columns.product_id'))
                                    ->columnSpan(3),
                                Forms\Components\TextInput::make('qty')
                                    ->live()
                                    ->label(trans('ecommerce.orders.columns.qty'))
                                    ->afterStateUpdated(function (Forms\Get $get, Forms\Set $set) {
                                        $product = Product::find($get('product_id'));
                                        if ($product) {
                                            $discount = 0;
                                ...
                                Forms\Components\TextInput::make('price')
                                    ->disabled()
                                    ->label(trans('ecommerce.orders.columns.price'))
                                    ->columnSpan(2)
                                    ->default(0)
                                    ->numeric(),
                                Forms\Components\TextInput::make('discount')
                                    ->disabled()
                                    ->label(trans('ecommerce.orders.columns.discount'))
                                    ->columnSpan(2)
                                    ->default(0)
                                    ->numeric(),
                                Forms\Components\TextInput::make('vat')
                                    ->disabled()
                                    ->label(trans('ecommerce.orders.columns.vat'))
                                    ->columnSpan(2)
                                    ->default(0)
                                    ->numeric(),
                                Forms\Components\TextInput::make('total')
                                    ->disabled()
                                    ->label(trans('ecommerce.orders.columns.total'))
                                    ->columnSpan(2)
                                    ->default(0)
                                    ->numeric(),
                            ])
                ...
                Forms\Components\Section::make(trans('ecommerce.orders.sections.totals'))
                    ->schema([
                        Forms\Components\Hidden::make('coupon_id'),
                        Forms\Components\TextInput::make('coupon')
                            ->label(trans('ecommerce.orders.columns.coupon'))
                            ->hidden(fn ($record) => $record)
                            ->suffixAction(
                                Forms\Components\Actions\Action::make('apply')
                                    ->tooltip(trans('ecommerce.orders.actions.apply'))
                                    ...
                                    ->action(function (Forms\Get $get, Forms\Set $set) {
                                        ...
                                        if ($coupon) {
                                            ...
                                            if ($getCouponDiscount) {
                                                ...
                                                Notification::make()
                                                    ->title(trans('ecommerce.orders.actions.coupon.success'))
                                                    ->success()
                                                    ->send();
                                            } else {
                                                Notification::make()
                                                    ->title(trans('ecommerce.orders.actions.coupon.not_valid'))
                                                    ->danger()
                                                    ->send();
                                            }
                                        } else {
                                            Notification::make()
                                                ->title(trans('ecommerce.orders.actions.coupon.not_found'))
                                                ->danger()
                                                ->send();
                                        }
                        ...
                        Forms\Components\TextInput::make('shipping')
                            ...
                            ->label(trans('ecommerce.orders.columns.shipping'))
                            ->numeric()
                            ->default(0),
                        Forms\Components\TextInput::make('vat')
                            ->disabled()
                        ...
                        Forms\Components\Toggle::make('has_returns')
                            ->label(trans('ecommerce.orders.columns.has_returns'))
                            ->live(),

                        Forms\Components\TextInput::make('return_total')
                            ->label(trans('ecommerce.orders.columns.return_total'))
                            ->hidden(fn (Forms\Get $get) => ! ($get('has_returns')))
                            ->numeric()
                            ->default(0),

                        Forms\Components\TextInput::make('reason')
                            ->label(trans('ecommerce.orders.columns.reason'))
                            ->hidden(fn (Forms\Get $get) => ! ($get('has_returns')))
                            ->maxLength(255),
                        Forms\Components\Textarea::make('notes')
                            ->label(trans('ecommerce.orders.columns.notes'))
                            ->columnSpanFull(),
                    ])->collapsible()->collapsed(fn ($record) => $record),
            ]);
    }
    ...
    public static function table(Table $table): Table
    {
        ...
        return $table
            ->headerActions([
                ExportAction::make()
                    ->hiddenLabel()
                    ->tooltip(trans('ecommerce.orders.actions.export'))
                    ->color('success')
                    ->icon('heroicon-s-document-arrow-down')
                    ->exporter(ExportOrders::class),
                Tables\Actions\Action::make('import')
                    ->hiddenLabel()
                    ->tooltip(trans('ecommerce.orders.actions.import'))
                    ->color('warning')
                    ->form([
                        Forms\Components\Textarea::make('data')
                            ->default("name: \nphone: \naddress: \nsource: \nitems: SKU*QTY,SKU*QTY")
                            ->hint(trans('ecommerce.orders.import.hint'))
                            ->label(trans('ecommerce.orders.import.order_text'))
                            ->autosize()
                            ->required(),
                    ])
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('created_at')
                    ->label(trans('ecommerce.orders.columns.created_at'))
                ...
                Tables\Columns\TextColumn::make('shipper.name')
                    ->label(trans('ecommerce.orders.columns.shipper_id'))
                    ->toggleable(isToggledHiddenByDefault: true)
                    ->sortable(),
                ...
                Tables\Columns\ToggleColumn::make('is_approved')
                    ->label(trans('ecommerce.orders.columns.is_approved'))
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
                Tables\Columns\ToggleColumn::make('is_closed')
                    ->label(trans('ecommerce.orders.columns.is_closed'))
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
                Tables\Columns\TextColumn::make('updated_at')
                    ->label(trans('ecommerce.orders.columns.updated_at'))
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
            ])
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('status')
                    ->label(trans('ecommerce.orders.filters.status'))
                    ->searchable()
                    ->options((clone $types)->pluck('name', 'key')->toArray()),
                Tables\Filters\Filter::make('company')
                    ->label(trans('ecommerce.orders.filters.company'))
                    ->form([
                        Forms\Components\Select::make('company_id')
                            ->label(trans('ecommerce.orders.filters.company'))
                            ->searchable()
                        ...
                        Forms\Components\Select::make('branch_id')
                            ->searchable()
                            ->options(fn (Forms\Get $get) => Branch::query()->where('company_id', $get('company_id'))->pluck('name', 'id')->toArray())
                            ->label(trans('ecommerce.orders.filters.branch_id')),
                    ])
                ...
                Tables\Filters\Filter::make('location')
                    ->label(trans('ecommerce.orders.filters.location'))
                    ->form([
                        Forms\Components\Select::make('country_id')
                            ->label(trans('ecommerce.orders.filters.country_id'))
                            ->searchable()
                            ->options(Country::query()->pluck('name', 'id')->toArray())
                            ->live(),
                        Forms\Components\Select::make('city_id')
                            ->label(trans('ecommerce.orders.filters.city_id'))
                            ->searchable()
                            ->options(fn (Forms\Get $get) => City::where('country_id', $get('country_id'))->pluck('name', 'id')->toArray()),
                        Forms\Components\Select::make('area_id')
                            ->label(trans('ecommerce.orders.filters.area_id'))
                            ->searchable()
                            ->options(fn (Forms\Get $get) => \TomatoPHP\FilamentLocations\Models\Area::where('city_id', $get('city_id'))->pluck('name', 'id')->toArray()),
                    ])
                    ->query(fn (Builder $query, array $data) => $query
                ...
                Tables\Filters\SelectFilter::make('account_id')
                    ->label(trans('ecommerce.orders.filters.account_id'))
                    ->searchable()
                    ->options(Account::pluck('name', 'id')->toArray()),
                Tables\Filters\SelectFilter::make('user_id')
                    ->label(trans('ecommerce.orders.filters.user_id'))
                    ->searchable()
                    ->options(User::pluck('name', 'id')->toArray()),

                Tables\Filters\SelectFilter::make('payment_method')
                    ->label(trans('ecommerce.orders.filters.payment_method'))
                    ->searchable()
                    ->options([
                        'cash' => 'Cash',
                        'credit' => 'Credit',
                ...
                Tables\Filters\TernaryFilter::make('is_approved')
                    ->label(trans('ecommerce.orders.filters.is_approved')),
                Tables\Filters\TernaryFilter::make('is_closed')
                    ->label(trans('ecommerce.orders.filters.is_closed')),
            ])
            ->actions([
                Tables\Actions\Action::make('approved')
                    ...
                    ->label(trans('ecommerce.orders.actions.approved'))
                    ->tooltip(trans('ecommerce.orders.actions.approved'))
                ...
                Tables\Actions\Action::make('shipping')
                    ...
                    ->form([
                        Forms\Components\Select::make('shipping_vendor_id')
                            ->label(trans('ecommerce.orders.columns.shipping_vendor_id'))
                            ->searchable()
                            ->live()
                    ...
                    ->label(trans('ecommerce.orders.actions.shipping'))
                    ->tooltip(trans('ecommerce.orders.actions.shipping'))
                ...
                Tables\Actions\Action::make('status')
                    ...
                    ->tooltip(trans('ecommerce.orders.actions.status'))
                    ->label(trans('ecommerce.orders.actions.status'))
                    ->icon('heroicon-s-adjustments-horizontal')
                    ->color('warning')
                    ->iconButton(),
                Tables\Actions\Action::make('print')
                    ->tooltip(trans('ecommerce.orders.actions.print'))
                    ->icon('heroicon-s-printer')
                    ->openUrlInNewTab()
                    ->url(fn ($record) => route('order.print', $record->id))
                    ->iconButton(),
                Tables\Actions\ViewAction::make()
                    ->tooltip(trans('ecommerce.orders.actions.show'))
                    ->iconButton(),
                Tables\Actions\EditAction::make()
                    ->tooltip(trans('ecommerce.orders.actions.edit'))
                    ->iconButton(),
            ])
