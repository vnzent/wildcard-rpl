...
class PostResource extends Resource
{
    ...
    {
        return trans('cms.content.posts.title');
    }

    public static function getLabel(): ?string
    {
        return trans('cms.content.posts.single');
    }
        ...
        return $form
            ->schema([
                Forms\Components\Grid::make([
                    ...
                    ->schema([
                        Forms\Components\Section::make(trans('cms.content.posts.sections.type.title'))
                            ->schema([
                                Forms\Components\ToggleButtons::make('type')
                                    ->label(trans('cms.content.posts.sections.type.columns.type'))
                                    ->live()
                                    ->options(CMSTypes::getOptions()->pluck('label', 'key')->toArray())
                                    ->icons(CMSTypes::getOptions()->pluck('icon', 'key')->toArray())
                                    ->colors(CMSTypes::getOptions()->pluck('color', 'key')->toArray())
                                    ->default('post')
                                    ->inline()
                                    ->columnSpanFull()
                                    ->hiddenLabel()
                        ...
                        Forms\Components\Grid::make()
                            ->schema(
                                [
                                    Forms\Components\Section::make(trans('cms.content.posts.sections.post.title'))
                                        ->description(trans('cms.content.posts.sections.post.description'))
                                        ->schema([
                                        ...
                                        ->columns(2),
                                    Forms\Components\Section::make(trans('cms.content.posts.sections.seo.title'))
                                        ->description(trans('cms.content.posts.sections.seo.description'))
                                        ->schema([
                                            Forms\Components\TextInput::make('short_description')->label(trans('cms.content.posts.sections.seo.columns.short_description')),
                                            Forms\Components\Textarea::make('keywords')->autosize()->label(trans('cms.content.posts.sections.seo.columns.keywords')),
                                        ]),
                                ]
                            )
                            ->columnSpan([
                        ...
                        Forms\Components\Grid::make()
                            ->schema([
                                Forms\Components\Section::make(trans('cms.content.posts.sections.status.title'))
                                    ->description(trans('cms.content.posts.sections.status.description'))
                                    ->schema([
                                        Forms\Components\Select::make('categories')
                                            ->hidden(fn (Forms\Get $get) => in_array($get('type'), ['page', 'builder']))
                                            ->relationship('categories', 'name')
                                            ->label(trans('cms.content.posts.sections.status.columns.categories'))
                                            ->createOptionForm([
                                        ...
                                        Forms\Components\Select::make('tags')
                                            ...
                                            ->label(trans('cms.content.posts.sections.status.columns.tags'))
                                            ->searchable()
                                            ->multiple()
                                            ->preload()
                                ...
                                Forms\Components\Section::make(trans('cms.content.posts.sections.images.title'))
                                    ->description(trans('cms.content.posts.sections.images.description'))
                                    ->schema([
                                        Forms\Components\SpatieMediaLibraryFileUpload::make('feature_image')
                                            ->label(trans('cms.content.posts.sections.images.columns.feature_image'))
                                        ...
                                        Forms\Components\SpatieMediaLibraryFileUpload::make('cover_image')
                                            ->label(trans('cms.content.posts.sections.images.columns.cover_image'))
                                        ...
                                        Forms\Components\SpatieMediaLibraryFileUpload::make('images')
                                            ->label(trans('cms.content.posts.sections.images.columns.images'))
                                            ->collection('images')
                                            ->multiple()
                                            ->image()
                                            ->maxSize(2048)
                                ...
                                Forms\Components\Section::make(trans('cms.content.posts.sections.author.title'))
                                    ->description(trans('cms.content.posts.sections.author.description'))
                                    ->schema([
                                        Forms\Components\Select::make('author_type')
                                            ->label(trans('cms.content.posts.sections.author.columns.author_type'))
    ...
    public static function infolist(Infolist $infolist): Infolist
    {
        return $infolist->schema([
            Section::make(trans('cms.content.posts.sections.post.title'))
                ->description(trans('cms.content.posts.sections.post.description'))
                ->schema([
            ...
            Grid::make([
                ...
                Section::make(trans('cms.content.posts.sections.status.title'))
                    ->description(trans('cms.content.posts.sections.status.description'))
                    ->schema([
                        ...
                        TextEntry::make('type')
                            ...
                            ->state(function (Post $post) {
                                return CMSTypes::getOptions()->where('key', $post->type)->first()?->label;
                            })
                            ->color(function (Post $post) {
                                return CMSTypes::getOptions()->where('key', $post->type)->first()?->color;
                            })
                            ->icon(function (Post $post) {
                                return CMSTypes::getOptions()->where('key', $post->type)->first()?->icon;
                            })
                            ->badge(),
                    ])->columnSpan(2),
                Section::make(trans('cms.content.posts.sections.seo.title'))
                    ->description(trans('cms.content.posts.sections.seo.description'))
                    ->schema([
                        TextEntry::make('short_description')
                            ->label(trans('cms.content.posts.sections.seo.columns.short_description')),
                        TextEntry::make('keywords')
                            ->label(trans('cms.content.posts.sections.seo.columns.keywords')),
                    ])->columnSpan(2),
            ]),
        ]);
    }
