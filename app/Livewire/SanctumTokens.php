        ...
        return $table
            ...
            ->columns([
                Tables\Columns\TextColumn::make('name')
                    ->label(trans('account.profile.token.name')),
                Tables\Columns\TextColumn::make('created_at')
                    ->date()
                    ->label(trans('account.profile.token.created_at'))
                    ->sortable(),
                Tables\Columns\TextColumn::make('expires_at')
                    ->color(fn ($record) => now()->gt($record->expires_at) ? 'danger' : null)
                    ->date()
                    ->label(trans('account.profile.token.expires_at'))
                    ->sortable(),
            ])
            ->actions([
            ...
            ->headerActions([
                Tables\Actions\CreateAction::make()
                    ->label(trans('account.profile.token.action_label'))
                    ->modalWidth('md')
                    ->form([
                        ...
                        CheckboxList::make('abilities')
                            ->label(trans('account.profile.token.abilities'))
                            ->options(function () {
                    ...
                    ->action(function ($data) {
                        ...
                        Notification::make()
                            ->success()
                            ->title(trans('account.profile.token.create_notification'))
                            ->send();
                    })
                    ->modalHeading(trans('account.profile.token.modal_heading')),
            ])
            ->emptyStateHeading(trans('account.profile.token.empty_state_heading'))
            ->emptyStateDescription(trans('account.profile.token.empty_state_description'));
    }
        ...
        return Action::make('token')
            ...
            ->form([
                TextInput::make('token')
                    ->label(trans('account.profile.token.token'))
                    ->helperText(trans('account.profile.token.helper_text')),
            ])
            ->modalHeading(trans('account.profile.token.modal_heading_2'))
