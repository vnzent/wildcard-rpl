@php
    if($getType() && $getFor()){
        $type = \App\Models\Type::where('key', $getState())
            ->where('for', $getFor())
            ->where('type', $getType())
            ->first();
    }
    else {
      $type = \App\Models\Type::where('key', $getState())->first();
    }
...
@endphp
@if($value || config('types.empty_state'))

    <span @if($description && $getAllowDescription()) x-tooltip="{content: '{{$description}}', theme: $store.theme }"
          @endif style="{{ implode([
    ...
    <div>
        {{ $value ?? config('types.empty_state') }}
    </div>
</span>
@endif
